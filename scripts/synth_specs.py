...
FUEL = 2000            # fuel budget the emitted theorems are stated at
INIT_FUEL = 5000       # fuel for `initGlobals`, matching scripts/differential.py
MAX_DOMAIN = 14        # cases per mined law: the kernel has to evaluate every one
...
def gen_candidates(core, byname, recs, rng):
    ...
    for name in core:
        ...
        def add(fam, source, law, note="", trivial=False, extra=None):
            c = Cand("%s_%s" % (fam, sid), fam, source, name, fdef, law, lits,
...
private def gp : Heap × Ref := initGlobals P %d Autoform.Generated.moduleInits
def h0 : Heap := gp.1
def gref : Ref := gp.2
...
def refute(cands, module, chunk=25):
    ...
    for i in range(0, len(live), chunk):
        part = live[i:i + chunk]
        src = [HEADER % (module, module, INIT_FUEL, FUEL)]
        for c in part:
...
def emit(cands, module, obligations_extra):
    ...
    out = [HEADER % (module, module, INIT_FUEL, FUEL)]
