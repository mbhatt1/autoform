...
namespace Autoform.SpecsGen.V8Base.Part50
...
-- REFUTED by computation: `lawIdempotent` does not hold for `f_v8_base_internal_RangeCheck___init__` over its own
...
theorem idempotent_v8_base_internal_RangeCheck___init_1_refuted : ((dom_idempotent_v8_base_internal_RangeCheck___init_1).all (lawIdempotent C FUEL f_v8_base_internal_RangeCheck___init__)) = false := by rfl


-- Holds at `FUEL` by kernel computation. The forall-fuel transport is recorded as
-- ob_commutes_v8_base_internal_RangeCheck___init_1 rather than proved; see the module header for the measured cost.

end Autoform.SpecsGen.V8Base.Part50
