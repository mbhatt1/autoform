...
| `op:arrayDecl:boxed-initializer` | The declaration of a *boxed* array/struct local with an initializer (or a whole-struct assignment to a boxed struct) outside the exactly-translated shapes: positional brace lists of integer values (zero-filling omitted integer slots) or explicit pointer values (null constant, function designator, plain string literal), ASCII string literals into integer arrays, and whole-struct copies from a side-effect-free same-typed source with scalar/pointer members. Designators, nested braces, float/aggregate elements, omitted pointer slots and values needing an implicit conversion stay here. Before this label existed the statement was silently dropped (`skip`), leaving every element `.unit`. | Not yet implemented (per shape). |
| `op:arrayDecl:partial-initializer` | `T a[N] = {v0, …, vk-1}` with `k < N` for an *unboxed* local array whose missing elements cannot be zero-filled exactly (non-integer element type, or more than 64 missing). Integer arrays are padded with `0` (C11 6.7.9p21); before, the `Val.list` simply ended at `k`, so `a[k]` raised `IndexError` instead of reading `0`. | Not yet implemented. |
| `op:arrayDecl:static-initializer` | The initializer of a *mutable* `static` local that is boxed. C initializes it once before the program runs; the box is re-created per call, so translating the initializer would silently discard writes made by earlier calls. (`static const` locals are translated.) | Blocked on persistent per-function storage (a module-level box). |
