...
The ledger (STRATEGY.md §3) is a statement of *what is proved, about what model, under
what assumptions*. It was previously ad-hoc text. This document specifies it as a **SACM
profile**: the model, the evidence types, and the rules for combining them into a claim.
`scripts/sacm.py` emits it.
...
Ours is the last mile only: the domain-specific evidence types below and their combination
rules.
...
Status says how strong the result was; `evidenceKind` says what kind of thing was done.
They are independent axes; collapsing them makes a green tick read as a proof.
...
A claim with `evidenceKind: TEST` is not a proved claim regardless of status. `proved` is
emitted on every claim node, and `G1` carries an explicit blocker saying faithfulness is
tested rather than proved. This is the SACM-side statement of what `Autoform/Ledger.lean`
prints as `NOT PROVED : transpiler faithfulness — see conformance.json`. `G2.1` is
`SUPPORTED; TEST`, and neither `G2` nor `G1` asserts that faithfulness is proved.
...
There is no averaging. Ties at rank 0 resolve to `DEFEATED` over `UNDEVELOPED`: a refuted
leg is worse news than an unexamined one.
...
The rule follows from an over-claim: `G2` read `SUPPORTED` off 104 agreeing cases
covering **92 of 238 functions** (a snapshot; the population was 238 when that
conformance run was taken and is now 209 — the ratio, not the denominator, is the point), a claim quantified over "the translated functions"
resting on evidence touching 39% of them. `divergences == 0` says nothing about the 146
functions no case reached.

The cap is not the preferred remedy. Where the exercised subject can be named, narrow the
claim to it and add a sibling goal for the remainder. Narrowing keeps the claim true;
capping the status of an over-broad claim leaves a false claim on the page with a hedge
attached. The cap is the fallback for when the covered subset cannot be named.
...
**R1 (faithfulness, `G2`).** Three separate questions, previously collapsed into one:

- *Did the compared cases agree?* `total == 0` → `UNSUPPORTED` (a vacuous oracle reports no
  evidence; this is `cachetools` before §19). `divergences > 0` → `DEFEATED`; a single
  divergence refutes the semantics on that corpus and nothing else in the case repairs it.
  `agree == total > 0` → threshold met.
...
- *What kind of evidence is it?* `TEST` (R9). It is how floored modulo, short-circuit
  evaluation, `-INT_MIN` and private name mangling were caught, but it is not a theorem.
...
untranslated construct is a premise the argument rests on, not an absence from it.
...
omitted; absence of a node must not be readable as absence of a problem. Conversely, R8/R9
